...
def insert_complaints_data(**kwargs):
    ...
    df_complaints_fact = pd.read_csv('/opt/airflow/clean_data/complaints_fact.csv', keep_default_na=False, na_values=['NA'])
    df_complaints_dim = pd.read_csv('/opt/airflow/clean_data/complaints_dim.csv', keep_default_na=False, na_values=['NA'])
    ...
    try:
        ...
        """
        query_insert = """
            INSERT INTO company_complaints_fact (companyName, industry, complaintSentDateId, complaintRecivedDateId, complaintId, companyId)
            VALUES %s
